 ...
 * Modified by arie nugraha to support new features and debugging
 *
 ...
 * Modified by Jushadi Arman Saz add gender in report 202508112241
 */
...
// PERBAIKAN: Menambahkan total anggota berdasarkan gender
$gender_report_q_male = $dbs->query('SELECT COUNT(member_id) FROM member WHERE gender = 1');
$gender_report_d_male = $gender_report_q_male->fetch_row();
$loan_report[__('Total Male Members')] = $gender_report_d_male[0];

$gender_report_q_female = $dbs->query('SELECT COUNT(member_id) FROM member WHERE gender = 0');
$gender_report_d_female = $gender_report_q_female->fetch_row();
$loan_report[__('Total Female Members')] = $gender_report_d_female[0];
